...
- (void)opencvEdgeDetectManual {
	...
	if(imageView.image) {
        ...
        cvAdaptiveThreshold(img, img2, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY_INV, 7, 20);
        
        //Find and draw Contours
        CvSeq* contours = 0;
        CvMemStorage* img_storage = cvCreateMemStorage(0);
        cvFindContours(img2, img_storage, &contours, sizeof(CvContour), CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cvPoint(0,0));
        cvDrawContours(img2, contours, cvScalarAll(255), cvScalarAll(0), 1, 1, CV_AA, cvPoint(0,0));
